...
export function allRGB(colors: string[]): boolean {
    ...
    const isNotRGB = (color: string): boolean =>
        color !== "red" && color !== "green" && color !== "blue";
    const noRGB = colors.filter(isNotRGB);
    if (noRGB.length === 0) {
        return true;
    } else {
        return false;
...
export function makeMath(addends: number[]): string {
    ...
    const sum = addends.reduce((nums, curr) => nums + curr, 0);
    const addendsString = addends.join("+");
    return sum.toString() + "=" + addendsString;
}
...
export function injectPositive(values: number[]): number[] {
    ...
    if (index === -1) {
        const total = values.reduce((totalSoFar, curr) => totalSoFar + curr, 0);
        return [...values, total];
    } else {
        const sum = values
            .slice(0, index)
            .reduce((totalSoFar, curr) => totalSoFar + curr, 0);
