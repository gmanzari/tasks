...
export function isCorrect(question: Question, answer: string): boolean {
    ...
    const expectedNoUpper = expected.toLowerCase();
    const answerNoUpper = answer.toLowerCase();
    const expectedTrimmed = expectedNoUpper.trim();
    const answerTrimmed = answerNoUpper.trim();
    return answerTrimmed === expectedTrimmed;
}
...
export function isValid(question: Question, answer: string): boolean {
    ...
    if (type === "short_answer_question") {
        ...
        const optionsCheck = question.options.filter(
...
export function toShortForm(question: Question): string {
    ...
    const first10 = name.substring(0, 10);
    return ID + ": " + first10;
}
...
export function toMarkdown(question: Question): string {
    ...
    if (type === "multiple_choice_question") {
        ...
        const option3 = question.options[2];
        return (
            "# " +
            name +
            "\n" +
...
export function publishQuestion(question: Question): Question {
    const newPublishedState = !question.published;
    const newQuestion = { ...question, published: newPublishedState };
    return newQuestion;
}
